...
Microsoft.WebPortal.Core.UrlManager.prototype.onPortalInitialized = function () {
    ...
    if (window.location.hash && window.location.hash.length > 0) {
        ...
        contextToPass = featureHash.length > 1 ? JSON.parse(featureHash[1]) : contextToPass;
    }
    ...
    window.onpopstate = function (event) {
        ...
        if (self.currentActiveFeatureIndex > event.state.index) {
            ...
            var retracted = self.webPortal.Journey.retract(self.currentActiveFeatureIndex - event.state.index - 1, context);
